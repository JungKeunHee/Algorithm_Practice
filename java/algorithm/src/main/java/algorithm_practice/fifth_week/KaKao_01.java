...
public class KaKao_01 {
    ...
    public static int stringCompression(String str) {
        int min = str.length(); // 최소 압축 길이 초기값 = 원본 문자열 길이

        // 단위 크기 1부터 str.length() / 2 까지만 시도 (그 이상은 압축 불가)
        for (int unit = 1; unit <= str.length() / 2; unit++) {
            StringBuilder compressed = new StringBuilder();
            String prev = str.substring(0, unit); // 처음 자른 단위
            int count = 1;
